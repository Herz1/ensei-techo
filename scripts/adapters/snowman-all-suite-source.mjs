...
const VENUE_IDS = new Map([
  ["札幌ドーム", "sapporo-dome"],
  ["バンテリンドーム ナゴヤ", "vantelin-dome"],
  ["みずほPayPayドーム福岡", "mizuho-paypay-dome"],
  ["東京ドーム", "tokyo-dome"],
...
export function createSnowManAllSuiteSourceAdapter(context) {
  const definition = {
    id: "snowman-all-suite-official",
    name: "Snow Man DOME TOUR 2026-2027 ALL SUITE 公式",
    type: "artist_official",
    role: "artist_official",
    parserIds: ["snowman-all-suite-html"],
    parserVersion: "1",
  ...
  return {
    ...
    async collect({ fetchText, recordParserResult, getRawEvidence }) {
      ...
      const parsed = parseSnowManAllSuite(html, definition.url, {
        months: context.months,
      });
      const raw = getRawEvidence(definition.url);
      await recordParserResult({
        url: definition.url,
        parserId: "snowman-all-suite-html",
      ...
      if (!overlapsTour) {
        return {
          skipped: true,
          reason: `目标 collection window 与 ALL SUITE 公演月份不重叠: ${parsed.tourMonths.join(", ")}`,
          records: [],
        };
      }
