...
const VENUE_IDS = new Map([
  ["幕張メッセ 国際展示場9-11ホール", "makuhari-messe-hall"],
  ["北九州メッセ", "kitakyushu-messe"],
]);
...
export function createVaundyHoroSourceAdapter(context) {
  const definition = {
    id: "vaundy-horo-official",
    name: "Vaundy ASIA ARENA TOUR 2026 HORO 日本公演 公式",
  ...
  return {
    ...
    async collect({ fetchText, recordParserResult, getRawEvidence }) {
      ...
      const parsed = parseVaundyHoro(html, definition.url, {
        months: context.months,
      });
      const raw = getRawEvidence(definition.url);
      await recordParserResult({
        url: definition.url,
        parserId: "vaundy-horo-japan-html",
      ...
      if (!overlapsTour) {
        return {
          skipped: true,
          reason: `目标 collection window 与日本公演月份不重叠: ${parsed.tourMonths.join(", ")}`,
          records: [],
        };
      }
