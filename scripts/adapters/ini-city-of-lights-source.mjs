...
export function createIniCityOfLightsSourceAdapter(context) {
  const definition = {
    id: "ini-city-of-lights-official",
    name: "2026 INI 5TH ANNIVERSARY DOME TOUR CITY OF LIGHTS 公式",
    type: "artist_official",
    role: "artist_official",
    parserIds: ["ini-city-of-lights-html"],
    parserVersion: "1",
    url: "https://ini-official.com/news/detail/4200",
  ...
  return {
    ...
    async collect({ fetchText, recordParserResult, getRawEvidence }) {
      ...
      const parsed = parseIniCityOfLights(html, definition.url, {
        months: context.months,
      });
      const raw = getRawEvidence(definition.url);
      await recordParserResult({
        url: definition.url,
        parserId: "ini-city-of-lights-html",
