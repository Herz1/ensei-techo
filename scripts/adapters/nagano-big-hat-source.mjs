...
export function createNaganoBigHatSourceAdapter(context) {
  const definition = {
    id: "nagano-big-hat-official",
    name: "長野ビッグハット 公式イベント情報",
    type: "venue_official",
    role: "venue_schedule",
    parserIds: ["nagano-big-hat-event-index-html", "nagano-big-hat-detail-html"],
    parserVersion: "1",
  ...
  return {
    ...
    async collect({ fetchText, recordParserResult, getRawEvidence }) {
      ...
      const discovered = parseNaganoBigHatScheduleLinks(indexHtml, definition.url, {
        knownArtistNames: context.knownArtistNames,
      });
      const indexRaw = getRawEvidence(definition.url);
      await recordParserResult({
        url: definition.url,
        parserId: "nagano-big-hat-event-index-html",
      ...
      for (const item of discovered.links) {
        try {
          ...
          const parsed = parseNaganoBigHatDetail(html, item.url, {
          ...
          await recordParserResult({
            url: item.url,
            parserId: "nagano-big-hat-detail-html",
