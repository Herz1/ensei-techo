...
const steps = [
  path.join(scriptDir, "pending-ingest-preflight.mjs"),
  path.join(scriptDir, "audit-pending-source-collisions.mjs"),
];
...
async function main() {
  ...
  if (process.env.PENDING_INGEST_PREFLIGHT_ONLY === "1") {
    console.log("Checked pending ingest: validation only, live ingest skipped");
    return;
  }
...
main().catch((error) => {
  console.error(
    `Checked pending ingest failed: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exitCode = 1;
});
